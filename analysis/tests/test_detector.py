...
from neuromorphicpaths_analysis.detector import (
    DECODER_SETTINGS,
    DecoderPreset,
    PathDetector,
    letterbox,
...
def test_real_model_finds_the_sidewalk_in_a_repository_photo(model_path: Path, test_photos_dir: Path) -> None:
    image = ImageOps.exif_transpose(Image.open(test_photos_dir / "IMG_2316.JPG")).convert("RGB")

    result = PathDetector(model_path).detect_path(image)
...
def test_every_decoder_preset_has_settings() -> None:
    assert set(DECODER_SETTINGS) == set(DecoderPreset)
    for preset, settings in DECODER_SETTINGS.items():
        assert settings.preset is preset
        assert DecoderPreset(preset.value) is preset
...
def test_outermost_edges_span_two_separate_runs() -> None:
    ...
    geometry = letterbox(Image.new("RGB", (1920, 1080)), 320, 320)[1]
    grid = numpy.zeros((GRID_SIZE, GRID_SIZE), dtype=bool)
    grid[40:60, 10:20] = True
    grid[40:60, 35:50] = True

    outermost = outermost_edges(grid, geometry)
    traced = trace_path(grid, geometry)

    assert {(row.left_cell, row.right_cell) for row in outermost} == {(10, 49)}
    assert {(row.left_cell, row.right_cell) for row in traced} == {(35, 49)}


def test_outermost_edges_ignore_cells_on_the_padding() -> None:
    geometry = letterbox(Image.new("RGB", (1920, 1080)), 320, 320)[1]
    grid = numpy.zeros((GRID_SIZE, GRID_SIZE), dtype=bool)
    grid[5:10, 30:50] = True  # above the image content, on the gray padding
    grid[40:45, 30:50] = True

    rows = outermost_edges(grid, geometry)

    assert sorted(row.grid_y for row in rows) == [40, 41, 42, 43, 44]


@pytest.mark.parametrize("preset", list(DecoderPreset))
def test_both_decoders_find_the_sidewalk_in_a_repository_photo(
    preset: DecoderPreset,
    model_path: Path,
    test_photos_dir: Path,
) -> None:
    image = ImageOps.exif_transpose(Image.open(test_photos_dir / "IMG_2319.JPG")).convert("RGB")

    result = PathDetector(model_path, preset).detect_path(image)

    assert result.grid_mask.any(), result.debug_info
    assert result.rows
