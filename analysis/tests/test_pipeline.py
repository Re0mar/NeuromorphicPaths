...
Uses IMG_2319.JPG from artifacts/images, an iPhone 12 ultra-wide shot of a straight footpath
...
PHOTO_PATH = REPOSITORY_ROOT / "artifacts" / "images" / "IMG_2319.JPG"
PROFILE_NAME = CaptureProfileName.IPHONE_12_ULTRA_WIDE

pytestmark = pytest.mark.skipif(
    not (DEFAULT_MODEL_PATH.exists() and PHOTO_PATH.exists()),
    reason="model file or test photo not in this checkout",
)


def run_geometry_command(*arguments: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, "-m", "neuromorphicpaths_analysis.geometry", *arguments, "--profile", PROFILE_NAME.value],
        capture_output=True,
        text=True,
        check=True,
    )
...
def test_real_photo_gives_a_reliable_point_of_view() -> None:
    result = PathDetector().detect_path(load_image(PHOTO_PATH))
...
def test_command_prints_and_saves_a_single_image(tmp_path: Path) -> None:
    csv_path = tmp_path / "pov.csv"

    completed = run_geometry_command(str(PHOTO_PATH), "--csv", str(csv_path))
...
def test_command_samples_a_video(tmp_path: Path) -> None:
    ...
    run_geometry_command(str(video_path), "--every", "2", "--csv", str(csv_path))
