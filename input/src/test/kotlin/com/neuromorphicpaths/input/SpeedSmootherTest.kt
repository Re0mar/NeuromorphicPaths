...
class SpeedSmootherTest {
    ...
    fun dopplerSpeedIsPreferredOverDistanceOverTime() {
        assertEquals(1.4, rawSpeedMetersPerSecond(1.4, 5.0, 1.0)!!, TOLERANCE)
    }

    @Test
    fun distanceOverTimeIsTheFallback() {
        assertEquals(1.5, rawSpeedMetersPerSecond(null, 3.0, 2.0)!!, TOLERANCE)
    }

    @Test
    fun noReadingWithoutDopplerOrAPreviousFix() {
        assertNull(rawSpeedMetersPerSecond(null, null, null))
        assertNull(rawSpeedMetersPerSecond(null, 3.0, 0.0))
    }
