...
> Written for the earlier version of the app, which followed a detected sidewalk. The push
> field in `push_field.md` grew out of the line and edge surprise derived here and uses the
> same units. The code paths named below belong to that earlier version and no longer exist
> in this tree. The math stands.
